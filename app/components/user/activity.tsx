							...
							<div className="mb-6 flex items-start justify-between gap-4">
								<div className="min-w-0">
									...
									>
										...
										{lobby.isWinner && (
											<span
												className="text-yellow-500 flex items-center shrink-0"
												title="Winner"
											>
												<CgTrophy size={18} />
											</span>
							...
							<div className="flex flex-col gap-3 mt-auto">
								...
								{(() => {
									...
									const waitMins = Math.max(
										0,
										Math.round((tSub - tCreated) / 60000),
									);
									const subMins = Math.max(
										0,
										Math.round((tVote - tSub) / 60000),
									);
									const voteMins = Math.max(
										0,
										Math.round((tEnded - tVote) / 60000),
									);
									...
									return (
										<div className="mt-2 flex flex-col gap-2">
											...
											<div className="flex text-[10px] uppercase font-bold tracking-wider text-gray-500">
												<span className="w-0 flex-1 text-left">
													{waitMins}m wait
												</span>
												<span className="w-0 flex-1 text-center">
													{subMins}m sub
												</span>
												<span className="w-0 flex-1 text-right">
													{voteMins}m vote
												</span>
											</div>
										</div>
