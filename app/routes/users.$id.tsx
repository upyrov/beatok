...
export async function clientLoader({
  ...
  const [user] = [
    queryClient.getQueryData<any>(userByIdQueryOptions(params.id!, year).queryKey),
  ];
...
export default function User() {
  ...
  if (!id) return <div className="p-8 text-center text-red-500">User not found</div>;
  if (!user && !isUserLoading) return <div className="p-8 text-center text-red-500">User not found</div>;
